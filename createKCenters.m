...
file_name = sprintf('%scodebook_%d.mat' , normalizeString, number_of_clusters);
...
function code_book_representraion = respresentUsingBagOfWords(cluster_centers, all_data, k_nearest_neighbors,number_of_elm_in_lower_level_pyramid)
    ...
    all_trail_code_book = zeros(number_of_trails, num_groups_in_pyramid, number_of_centers*k_nearest_neighbors,'int16');
    for i=1:number_of_trails
        current_trail = squeeze( all_data(i,:,:) );
        current_trail_code_book = zeros(num_groups_in_pyramid, number_of_centers*k_nearest_neighbors,'int16');
        for j = 1:num_groups_in_pyramid
            ...
            current_trail_code_book(j,:) = indices_of_selected_centers;
        end
    ...
    code_book_representraion = reshape(code_book_representraion, number_of_trails, num_groups_in_pyramid * number_of_centers );
end
